    ...
    for _ in range(num_epochs):
        for X, y in data_iter:
            ...
            if n % 200 == 0:
                ...
                timer.start()
    print(
        f'loss: {animator.Y[0][-1]:.3f}, {timer.sum()/num_epochs:.3f} sec/epoch')
...
class HPOTuner(d2l.HyperParameters):
    ...
    def bookkeeping(self, config: dict, error: float, runtime: float):
        """Defined in :numref:`sec_api_hpo`"""
        self.records.append(
            {"config": config, "error": error, "runtime": runtime})
...
def frozen_lake(seed):
    ...
    # Number of observations/states or obs/state dim
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device, init_weight=True):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            ...
            # Y_hat.argmax(dim=2).cpu().numpy()[0].tolist()
            # Y.detach().cpu().numpy()[0].tolist()4120
            optimizer.step()
            with torch.no_grad():
                metric.add(l.sum(), l.shape[0])
        if (epoch + 1) % 10 == 0:
            animator.add(epoch + 1, (metric[0] / metric[1],))
            print(
                f'epoch: {epoch}/{num_epochs}, loss: {metric[0] / metric[1]}')
    print(f'finished. loss {metric[0] / metric[1]:.3f}, {metric[1] / timer.stop():.1f} '
          f'tokens/sec on {str(device)}')


def predict_seq2seq(net, src_sentence, src_vocab, tgt_vocab, num_steps,
                    device, save_attention_weights=False, tokenize=lambda x: x.lower().split(' ')):
